...
//		contact[i].surface.mu = 5.0; // ODE manual says mu \approx 1 (!)
		contact[i].surface.mu = 50.0;
//		contact[i].surface.mu = 300.0;  // TBD: higher friction coeff
						// for pneu
...
void Simulate(int pause) {
	...
	if ( !pause ) {
		...
		else if ( envs->In_File_Mode() )

			envs->Show_From_File(	world, 
						space,
						cntrlrFN);
...
void Switch_Mode(char cmd) {
	...
	// Switch to controller file mode.
	else if ( cmd=='$' )
		if ( cntrlrFN )
			envs->Mode_Simulate_Set_File();
}
...
void Print_Usage(void) {
	...
	printf("   #: assign joint group ID to marked joints\n");
	...
	cntrlrRead = false;
		...
		if ( strcmp(argv[currParam],"-cscript") == 0 )
			captureScript = true;
                ...
                if ( strcmp(argv[currParam],"-fc") == 0 ) {
                        cntrlrFN = argv[currParam+1];
			printf("controller filename to read: %s\n", cntrlrFN);
			cntrlrRead = true;
		}

		// TBD: cmd line env file read not implemented
                if ( strcmp(argv[currParam],"-fenv") == 0 ) {
                        envFN = argv[currParam+1];
			printf("envfilename to read: %s\n", envFN);
		}

		// TBD: cmd line envs file read not implemented
                if ( strcmp(argv[currParam],"-fenvs") == 0 ){
                        envsFN = argv[currParam+1];
			printf("envsfilename to read: %s\n", envsFN);
		}
