...
void ENVIRONMENT::Add_Robot_Pneu_Quad(void) {

	// parts for user-created robots

	if ( numRobots < MAX_ROBOTS ) {

		robots[numRobots] = new ROBOT(this,ROBOT_PNEU_QUAD);
...
void ENVIRONMENT::Group_Robot_Joints(void) {

	if ( numRobots < 1 )
		return;

	robots[robotIndex]->Group_Robot_Joints();
}
...
void ENVIRONMENT::Label(CONTROLLER *genome, int environmentIndex) {
...
void ENVIRONMENT::Label_From_File(CONTROLLER *genome, int environmentIndex, char* fileName) {

	if ( robots[0] )

		robots[0]->Label_From_File(genome,environmentIndex,fileName);
}
...
void ENVIRONMENT::Load(void) {
	...
	printf("environment: loading file %s\n",fileName);
