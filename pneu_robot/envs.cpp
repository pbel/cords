...
//	taskEnvironments[0]->Add_Robot_Sandbox();
	taskEnvironments[0]->Add_Robot_Pneu_Quad();
...
//		else if ( selectionLevel == SELECTION_LEVEL_ROBOT )
		...
		if ( selectionLevel == SELECTION_LEVEL_ROBOT )
			...
			taskEnvironments[activeEnvironment]->Group_Robot_Joints();
}
...
//	optimizer->Reset_But_Keep_Best(); // TBD: doesn't exist
...
int ENVS::In_File_Mode(void) {

	return( simulateMode == MODE_SIMULATE_FILE );
}
...
void ENVS::Mode_Simulate_Set_File(void) {

	// Switch to visualizing the best robot so far.

	// Already in file mode.
	if ( In_File_Mode() )
	...
	// TBD: for a controller from a file, it shouldn't be necessary to
	// create a robot
	if ( In_Design_Mode() )
	...
	simulateMode = MODE_SIMULATE_FILE;

	// robots with a controller from a file are cyan
	for (int i=0;	i<numberOfEnvs;	i++) {

		taskEnvironments[i]->Set_Color(0.0,0.6,0.6);

		taskEnvironments[activeEnvironment]->Robots_Recolor('c');
	}
}
...
void ENVS::Show_From_File(	dWorldID      world, 
				dSpaceID      space,
				char*	      fileName) {

	// If there is no robot, create one.
	if ( !Robot_Being_Evaluated() )

		Create_Robot_From_Controller_File(world,space,fileName);
	...
	// If the time limit for the robot has
	// expired, replay it.
	else {
...
void ENVS::Create_Robot_From_Controller_File( 	dWorldID      world, 
						dSpaceID      space,
						char*	      fileName) {

	currNumberOfEnvs = numberOfEnvs;

	CONTROLLER *genome = optimizer->Genome_Get_Curr_To_Evaluate();

	for (int i=0;	i<currNumberOfEnvs;	i++) {

		taskEnvironments[i]->Prepare_For_Simulation(world,space);

		taskEnvironments[i]->Label_From_File(genome,i,fileName);
	}
...
//	NEURAL_NETWORK *bestGenome = optimizer->Genome_Get_Best(currNumberOfEnvs);
	CONTROLLER *bestGenome = optimizer->Genome_Get_Best(currNumberOfEnvs);
...
//	NEURAL_NETWORK *nextGenome = optimizer->Genome_Get_Next_To_Evaluate(&currNumberOfEnvs);
	CONTROLLER *nextGenome = optimizer->Genome_Get_Next_To_Evaluate(&currNumberOfEnvs);
...
void ENVS::Optimizer_Initialize(void) {
	...
	if ( !optimizer ) {
		...
		int numberOfTimeSteps = 
			taskEnvironments[0]->robots[0]->Get_Num_Time_Steps(); 

		// TBD: use motorGroups for pneumatic robot instead of motors
		// and no hidden nodes; not a neural network
		// TBD: numMotorGroups is more general than numMotors; use it always?
		int numberOfMotorGroups = 
			taskEnvironments[0]->robots[0]->Get_Num_Motor_Groups(); 

		// TBD: one joint group is static (leg-to-body)	
		// TBD: goes away when composite parts replace these joints
		numberOfMotorGroups--;
...
int ENVS::Optimizer_Robot_Mismatch(void) {
	...
	return ( optimizer->numSensors != 
	     	taskEnvironments[0]->robots[0]->Sensors_Number_Of()
	     || optimizer->numTimeSteps !=
		 taskEnvironments[0]->robots[0]->Get_Num_Time_Steps()
	     || optimizer->numMotorGroups !=
		 taskEnvironments[0]->robots[0]->Get_Num_Motor_Groups() );
}
...
//	(*outFile) << "mencoder 'mf://*.jpg' -mf fps=60 -o Movie"<<movieIndex
//		   <<".mp4 -ovc lavc -lavcopts vcodec=mpeg4 -of mpeg \n";
