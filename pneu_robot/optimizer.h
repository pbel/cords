...
private:
	...
	CONTROLLER *genomeUnderEvaluation;
...
public:
	OPTIMIZER(int numberOfSensors, int numberOfTimeSteps,
		  int numberOfMotors);
	...
	CONTROLLER *Genome_Get_Best(int maxEnvironments);
	CONTROLLER *Genome_Get_Curr_To_Evaluate(void);
	CONTROLLER *Genome_Get_Next_To_Evaluate(int *envsToEvaluateIn);
...
private:
	...
	CONTROLLER *Genome_Copy(int parentID);
	...
	CONTROLLER *Genome_Find_Next_Not_Evaluated(void);
	CONTROLLER *Genome_Get(int layerIndex, int genomeIndex);
	...
	CONTROLLER *Layer_Most_Fit_Genome(int layerIndex);
