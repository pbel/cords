...
OPTIMIZER::OPTIMIZER(int numberOfSensors, int numberOfTimeSteps, 
	...
	numMotorGroups 	= numberOfMotorGroups;
}
...
CONTROLLER *OPTIMIZER::Genome_Get_Best(int maxEnvironments) {
...
CONTROLLER *OPTIMIZER::Genome_Get_Curr_To_Evaluate(void) {
...
CONTROLLER *OPTIMIZER::Genome_Get_Next_To_Evaluate(int *envsToEvaluateIn) {

	CONTROLLER *genomeToReturn;
...
CONTROLLER *OPTIMIZER::Genome_Copy(int parentID) {

	CONTROLLER *temp = new CONTROLLER(genomes[parentID]);
...
//	printf("optimizer: numTimeSteps = %d, numMotorGroups = %d\n",
//	       numTimeSteps, numMotorGroups); 

	genomes[Genome_Index(layerIndex,genomeIndex)] = 

		new CONTROLLER(nextGenomeID++,numSensors,numTimeSteps,numMotorGroups,genomesEvaluated);
}
...
CONTROLLER *OPTIMIZER::Genome_Find_Next_Not_Evaluated(void) {
...
CONTROLLER *OPTIMIZER::Genome_Get(int layerIndex, int genomeIndex) {
...
CONTROLLER *OPTIMIZER::Genome_Spawn(void) {
...
void OPTIMIZER::Genomes_Create(void) {
	...
	genomes = new CONTROLLER * [ ALPS_NUM_LAYERS * ALPS_GENOMES_PER_LAYER ];
...
CONTROLLER *OPTIMIZER::Layer_Most_Fit_Genome(int layerIndex) {
...
void OPTIMIZER::TryMoveUp(int layerIndx) {
	...
	if ( victimIndex > -1 ) {
		...
		if ( Layer_Top(layerIndx) ) {
			...
			CONTROLLER *temp = genomes[victimIndex];
			genomes[victimIndex] = genomeUnderEvaluation;
